...
func textCtx(t *testing.T, src string) checks.Context {
	t.Helper()
	return checks.Context{FilePath: "notes/n.md", Doc: mustParseDoc(t, src)}
}

func TestTextForbids_findsAndLocatesMatch(t *testing.T) {
	// Body starts at line 4; "second TODO" is line 5.
	ctx := textCtx(t, "---\ntitle: T\n---\nfirst line\nsecond TODO line\n")
	v := checks.TextForbids{Re: regexp.MustCompile(`\bTODO\b`), Pattern: `\bTODO\b`}.Run(ctx)
	if len(v) != 1 {
...
func TestTextForbids_passesWhenAbsent(t *testing.T) {
	ctx := textCtx(t, "---\ntitle: T\n---\nall clear here\n")
	if v := (checks.TextForbids{Re: regexp.MustCompile(`\bTODO\b`), Pattern: `TODO`}).Run(ctx); len(v) != 0 {
...
func TestTextForbids_perLineReportsEachSpan(t *testing.T) {
	ctx := textCtx(t, "---\nt: x\n---\nTODO one\nclean\nTODO two\n")
	v := checks.TextForbids{Re: regexp.MustCompile(`TODO`), Pattern: `TODO`, Target: "line"}.Run(ctx)
	if len(v) != 2 {
...
func TestTextRequires_anyMatchesOnOneSpan(t *testing.T) {
	ctx := textCtx(t, "---\nt: x\n---\nintro\n## Sources\n- a\n")
	if v := (checks.TextRequires{Re: regexp.MustCompile(`Sources`), Pattern: `Sources`, Target: "line"}).Run(ctx); len(v) != 0 {
		t.Fatalf("any: expected pass, got %d violations", len(v))
	}
	ctx2 := textCtx(t, "---\nt: x\n---\nintro only\n")
	v := checks.TextRequires{Re: regexp.MustCompile(`Sources`), Pattern: `Sources`}.Run(ctx2)
	if len(v) != 1 {
...
func TestTextRequires_allRequiresEverySpan(t *testing.T) {
	...
	pass := textCtx(t, "---\nt: x\n---\nx1\nx2\n")
	if v := (checks.TextRequires{Re: regexp.MustCompile(`x`), Pattern: `x`, Target: "line", All: true}).Run(pass); len(v) != 0 {
		t.Fatalf("all: expected pass, got %d violations", len(v))
	}
	fail := textCtx(t, "---\nt: x\n---\nx1\ny2\n")
	v := checks.TextRequires{Re: regexp.MustCompile(`x`), Pattern: `x`, Target: "line", All: true}.Run(fail)
	if len(v) != 1 {
...
func TestTextRequires_anyEqualsAllForBody(t *testing.T) {
	...
	any := checks.TextRequires{Re: regexp.MustCompile(`Sources`), Pattern: `Sources`, Target: "body"}.Run(ctx)
	all := checks.TextRequires{Re: regexp.MustCompile(`Sources`), Pattern: `Sources`, Target: "body", All: true}.Run(ctx)
...
func TestTextDenylist_flagsLiteralsMetacharsInert(t *testing.T) {
	...
	v := checks.TextDenylist{Values: []string{"TODO", "FIXME"}}.Run(ctx)
	if len(v) != 1 {
		t.Fatalf("expected 1 violation, got %d", len(v))
	}
	...
	inert := textCtx(t, "---\nt: x\n---\naxb\n")
	if v := (checks.TextDenylist{Values: []string{"a.b"}}).Run(inert); len(v) != 0 {
		t.Fatalf("metachar should be inert; got %d violations", len(v))
	}
	literal := textCtx(t, "---\nt: x\n---\na.b\n")
	if v := (checks.TextDenylist{Values: []string{"a.b"}}).Run(literal); len(v) != 1 {
...
func TestTextSpans_firstLineSkipsLeadingBlanks(t *testing.T) {
	...
	v := checks.TextForbids{Re: regexp.MustCompile(`Headline`), Pattern: `Headline`, Target: "first-line"}.Run(ctx)
...
func TestTextSpans_matchedLinesUsesSelect(t *testing.T) {
	...
	v := checks.TextForbids{Re: regexp.MustCompile(`TODO`), Pattern: `TODO`, Target: "matched-lines", Select: sel}.Run(ctx)
	if len(v) != 1 {
...
func TestTextForbids_frontmatterlessFileLintsFromLineOne(t *testing.T) {
	...
	v := checks.TextForbids{Re: regexp.MustCompile(`TODO`), Pattern: `TODO`}.Run(ctx)
...
func TestTextForbids_bodyTargetReportsMatchLine(t *testing.T) {
	...
	ctx := textCtx(t, "---\nt: x\n---\nfirst\nsecond TODO\nthird\n")
	v := checks.TextForbids{Re: regexp.MustCompile(`TODO`), Pattern: `TODO`, Target: "body"}.Run(ctx)
