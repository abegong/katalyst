...
orchestration hub the `check` lifecycle runs from.
...
code-level contract is `go doc ./internal/config`. This file keeps only the
...
- `config` imports `internal/query` for the variant `when` predicate grammar;
  `query` imports no `config`, so there is no cycle.
