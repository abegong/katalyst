...
// docs/reference/inspectors/ from internal/inspect/registry.go. It renders the
// worked-examples reference under docs/reference/examples/ and their embeddable
// snippets under docs/generated/examples/ from internal/examples, running each
// example's command so the published output is real. It also mirrors the
...
// examplesOut is the generated worked-examples section, relative to the repo
// root. examplesSnippetsOut holds the raw command-output snippets the
// {{< katalyst-example >}} shortcode embeds into prose pages; it lives outside
// content/ so Hugo does not render it and katalyst does not check it.
const (
...
func run() error {
	...
	// Worked examples: render the catalog section and the embeddable output
	// snippets from the example registry, running each example so the published
	// output is the tool's real output.
...
const examplesGeneratedNote = "<!-- GENERATED by cmd/gendocs from internal/examples (the command output is real). Do not edit by hand; run `make docs-gen`. -->"

// writeExamples renders the worked-examples catalog under examplesOut and the
// embeddable output snippets under examplesSnippetsOut. Each example's command
// is actually run, so the docs carry the tool's real output; the same Run is
// gated by internal/examples' test, so the output is a tested contract.
func writeExamples() error {
	...
	exs := examples.All()
	if err := write(filepath.Join(examplesOut, "_index.md"), examplesIndex(exs)); err != nil {
		return err
	}
	for _, ex := range exs {
		...
		if err := write(filepath.Join(examplesOut, ex.ID+".md"), examplePage(ex, res)); err != nil {
			return err
		}
		// The snippet is plain text (the command's raw output), embedded
		// verbatim by the shortcode; no Hugo frontmatter.
...
func examplesIndex(exs []examples.Example) string {
	var b strings.Builder
	fmt.Fprint(&b, "+++\ntitle = \"Examples\"\nweight = 50\nbookCollapseSection = true\n+++\n\n")
	fmt.Fprintln(&b, examplesGeneratedNote)
	fmt.Fprint(&b, "\n# Worked examples\n\n")
	fmt.Fprint(&b, "Each example pairs a small input corpus with a `katalyst` command and shows ")
	fmt.Fprint(&b, "the real output. They are generated by running the command and gated by tests, ")
	fmt.Fprint(&b, "so the output here cannot drift from the shipped tool.\n\n")
	for _, ex := range exs {
		fmt.Fprintf(&b, "- [%s]({{< relref \"%s.md\" >}}): %s\n", ex.Title, ex.ID, ex.Summary)
	}
	return b.String()
}

func examplePage(ex examples.Example, res examples.Result) string {
	var b strings.Builder
	fmt.Fprintf(&b, "+++\ntitle = \"%s\"\nweight = %d\n+++\n\n", ex.Title, ex.Weight)
	fmt.Fprintln(&b, examplesGeneratedNote)
	fmt.Fprint(&b, "\n")
	b.WriteString(examples.RenderPage(ex, res))
	return b.String()
}
